...
- **改系统控件的外观，必须写完整 `ControlTemplate`。** 只设 `Background` / `Foreground` 是盖不住的
  ——WPF 默认模板自带一套 chrome（`ContextMenu` / `MenuItem` 默认就是浅色的），
  踩过一次：深色盒子上弹出**纯白菜单**，用户当场截图骂回来。
  三条相关的坑一起记住：
  1. `ContextMenu` 的模板里**必须保留 `PART_Popup`**——菜单靠它才能打开，它同时是子菜单的宿主，删了就弹不出来；
  2. `ContextMenu` **不能挂进视觉树**（抛「ContextMenu 不能有逻辑或视觉父级」），离屏渲染要先转成位图；
  3. 系统控件默认 `Visibility` 可能是 `Collapsed`（`ContextMenu` 就是），量尺寸前要显式打开。
